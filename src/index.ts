...
const get_user_name = document.querySelector("#user") as HTMLInputElement;
const form_submit = document.querySelector("#form") as HTMLFormElement;
let main_container = document.querySelector(".main-container") as HTMLElement ;

// So let's define the contract of an Object
interface UserData {
  id: number;
  login: string;
  avatar_url: string;
...
async function myCustomFetcher<T>(
  ...
  form_submit.addEventListener("submit", async (e) => {
    ...
    const searchTerm = get_user_name.value.toLowerCase();
